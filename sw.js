...
self.addEventListener('activate', function(e) {
  e.waitUntil(
    ...
    .then(function(clients) {
      clients.forEach(function(cl) { cl.navigate(cl.url); });
    })
  );
});
...
self.addEventListener('fetch', function(e) {
  ...
  e.respondWith(
    fetch(e.request).then(function(r) {
      if (r && r.status === 200) {
        caches.open(CACHE).then(function(ca) { ca.put(e.request, r.clone()); });
      }
      return r;
    }).catch(function() {
...
console.log('[KM-SW] v60 actif — www.kassanmou.net');
